...
async function parseFigma(args) {
...
async function analyzeScreenshot(args) {
...
function generateReactComponent(layout, includeA11y) {
    return `
...
function generateSvelteComponent(layout, includeA11y) {
    return `
<div ${includeA11y ? 'role="region" aria-label="Component"' : ''}>
  <h1>Generated Component</h1>
...
function generateVueComponent(layout, includeA11y) {
    return `
<template>
  <div ${includeA11y ? 'role="region" aria-label="Component"' : ''}>
    <h1>Generated Component</h1>
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        { name: 'parse_figma', description: 'Parse Figma JSON export', inputSchema: zodToJsonSchema(ParseFigmaSchema) },
        { name: 'analyze_screenshot', description: 'Analyze screenshot layout', inputSchema: zodToJsonSchema(AnalyzeScreenshotSchema) },
        { name: 'generate_component', description: 'Generate code component', inputSchema: zodToJsonSchema(GenerateComponentSchema) }
    ]
}));
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        let result;
        if (name === 'parse_figma')
            result = await parseFigma(ParseFigmaSchema.parse(args));
        else if (name === 'analyze_screenshot')
            result = await analyzeScreenshot(AnalyzeScreenshotSchema.parse(args));
        else if (name === 'generate_component')
            result = await generateComponent(GenerateComponentSchema.parse(args));
        else
